...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        for i in range (self.iterations_):
            ...
            ep = np.sum(self.D_[y_pred != y])
            if ep == 0:
                break
            self.weights_[i] = 0.5 * np.log((1 - ep) / ep)
    ...
    def _predict(self, X):
        ...
        """
        return np.sign(np.sum(np.array([model.predict(X) * weight for model, weight in zip(self.models_, self.weights_)]), axis=0))
    ...
    def partial_predict(self, X: np.ndarray, T: int) -> np.ndarray:
        ...
        responses : ndarray of shape (n_samples, )
            Predicted responses of given samples
        """
        return np.sign(np.sum(np.array([model.predict(X) * weight for model, weight in zip(self.models_[:T], self.weights_[:T])]), axis=0))
